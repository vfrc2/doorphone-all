...
class TagHandler:
    ...
    def handleType4ATag(self, tag: nfc.tag.tt4.Type4Tag ):
        ...
        try:
            ...
            resRaw = tag.send_apdu(cla, ins, p1, p2, data, check_status=True)
            res = self.__byteArrayToHexString(resRaw)

            resRaw2 = tag.send_apdu(cla, 0xA5, 0, 0, data, check_status=True)
            uid = self.__byteArrayToHexString(resRaw2)
            self.callback('Type4ATag', uid)
        except nfc.tag.tt4.Type4TagCommandError as err:
            print(err)
